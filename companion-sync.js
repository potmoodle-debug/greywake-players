(() => {
  ...
  function localState(){const s=window.GreywakeCompanion?.getState?.();if(!s)return null;return{hope:Number(s.hope),maxHope:Number(s.maxHope),stress:Number(s.stress),maxStress:Number(s.maxStress),hp:Number(s.hp),maxHP:Number(s.maxHP),effects:s.effects||{},strangePattern:Number(s.strangePattern||1)};}
  ...
  function same(a,b){return a&&b&&['hope','maxHope','stress','maxStress','hp','maxHP','strangePattern'].every(k=>Number(a[k]||0)===Number(b[k]||0))&&JSON.stringify(a.effects||{})===JSON.stringify(b.effects||{});}
  ...
  function applyRemote(remote){const api=window.GreywakeCompanion,current=localState();if(!api||!current||!valid(remote)||same(current,remote))return;applying=true;try{api.importState(remote);}finally{applying=false;}}
  ...
  async function pull({initial=false}={}){
    ...
    try{
      const bundle=await request('GET'),remote=parseLatest(bundle);if(!remote)throw new Error('Resource sync record missing.');
      remoteRevision=Math.max(remoteRevision,Number(remote.revision)||0);
      const local=localState(),untouched=remote.source==='initial_import'&&Number(remote.revision)<=1;
      if(initial&&!isPreview()&&untouched&&local&&!same(local,remote)){lastServerMessageId=Number(remote.messageId)||0;await push('First cross-device sync','local_migration');return;}
      if(Number(remote.messageId)>lastServerMessageId||initial){applyRemote(remote);lastServerMessageId=Number(remote.messageId)||lastServerMessageId;}
      syncLabel(isPreview()?'GM preview · local test · not synced':'Synced across devices','ok');
    }catch(error){syncLabel('Local state · sync will retry','error');console.warn('Greywake companion sync pull failed:',error);}
  }

  async function push(reason='Resource update',source='client'){
    if(!activeKey||isPreview()||applying)return;
    const s=localState();if(!s)return;
    try{
      if(!systemGoalId){const remote=parseLatest(await request('GET'));if(!remote||!systemGoalId)throw new Error('Resource sync record unavailable.');remoteRevision=Math.max(remoteRevision,Number(remote.revision)||0);lastServerMessageId=Math.max(lastServerMessageId,Number(remote.messageId)||0);}
      const payload={v:1,...s,revision:remoteRevision+1,source,reason:String(reason||'Resource update').slice(0,120),at:new Date().toISOString()};
      const result=await request('POST',{goal_id:systemGoalId,message:JSON.stringify(payload),kind:'reply'});remoteRevision=payload.revision;if(result?.message?.id)lastServerMessageId=Number(result.message.id);pushPending=false;syncLabel('Synced across devices','ok');
    }catch(error){pushPending=true;syncLabel('Local change waiting to sync','error');console.warn('Greywake companion sync push failed:',error);}
  }
  ...
  async function init(){
    const k=key();if(!k){activeKey=null;stopPolling();return;}
    if(activeKey!==k){activeKey=k;systemGoalId=null;lastServerMessageId=0;remoteRevision=0;pushPending=false;initialized=false;}
    if(!window.GreywakeCompanion||window.GreywakeCompanion.key!==k){setTimeout(init,120);return;}
    stopPolling();await pull({initial:true});if(!isPreview())startPolling();initialized=true;
  }

  window.addEventListener('greywake:companion-resources-changed',schedulePush);
  window.addEventListener('greywake:player-ready',()=>setTimeout(init,160));
