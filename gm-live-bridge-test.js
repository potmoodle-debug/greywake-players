(() => {
  ...
  function ensureStyles(){
    ...
    const s=document.createElement('style');s.id='gm-live-bridge-test-styles';s.textContent=`
      ...
      @media(max-width:900px){.gm-bridge-delivery{grid-template-columns:1fr 1fr}}
      @media(max-width:700px){.gm-bridge-status,.gm-bridge-delivery{grid-template-columns:1fr}.gm-bridge-test-head{display:block}.gm-bridge-badge{display:inline-flex;margin-top:8px}.gm-bridge-chat{grid-template-columns:1fr}}
  ...
  function statusLabel(){
    ...
    if(s==='queued')return['QUEUED','warn'];
    if(s==='processing')return['SENDING','warn'];
    if(s==='sent')return['SENT','good'];
    if(s==='accepted')return['ACCEPTED','good'];
    if(s==='duplicate')return['ALREADY SENT','good'];
    if(s==='blocked')return['BLOCKED','bad'];
    return['READY',''];
  }
  ...
  function mount(){
    ...
    const live=liveChat(),updater=updaterChat(),configured=!!live&&!!updater&&live.key!==updater.key,[deliveryLabel,deliveryClass]=statusLabel();
