(() => {
  ...
  function localState() {
    const state = window.GreywakeResources?.getState?.();
    if (!state) return null;
    return {
      hope: Number(state.hope), maxHope: Number(state.maxHope),
      stress: Number(state.stress), maxStress: Number(state.maxStress),
      hp: Number(state.hp), maxHP: Number(state.maxHP)
    };
  }

  function validRemote(value) {
    if (!value || Number(value.v) !== 1) return false;
    const pairs = [['hope','maxHope'],['stress','maxStress'],['hp','maxHP']];
    return pairs.every(([current,max]) => Number.isInteger(Number(value[current])) && Number.isInteger(Number(value[max])) && Number(value[max]) > 0 && Number(value[current]) >= 0 && Number(value[current]) <= Number(value[max]));
  }

  function sameState(a, b) {
    return a && b && ['hope','maxHope','stress','maxStress','hp','maxHP'].every(key => Number(a[key]) === Number(b[key]));
  }
  ...
  function applyRemote(remote) {
    ...
    applyingRemote = true;
    document.body.classList.add('resource-sync-applying');
    try {
      ...
      document.body.classList.remove('resource-sync-applying');
      applyReadOnly();
    }
  }
  ...
  async function pull({ initial = false } = {}) {
    ...
    try {
      ...
      if (Number(remote.messageId) > lastServerMessageId) {
        applyRemote(remote);
        lastServerMessageId = Number(remote.messageId) || lastServerMessageId;
      }
      syncLabel(isPreview() ? 'GM preview · live synced state · read only' : 'Synced across devices', 'ok');
    } catch (error) {
  ...
  function schedulePush(event) {
    ...
    const reason = event?.detail?.reason || 'Resource update';
    pushTimer = setTimeout(() => push(reason), 450);
  }
  ...
  function applyReadOnly() {
    ...
    document.querySelectorAll(READONLY_SELECTOR).forEach(node => {
      if (readonly) {
        if (!node.disabled) node.dataset.syncDisabled = 'true';
        node.disabled = true;
      } else if (node.dataset.syncDisabled === 'true') {
        node.disabled = false;
        delete node.dataset.syncDisabled;
      }
    });
    if (readonly) syncLabel('GM preview · live synced state · read only', 'readonly');
  }

  function startPolling() {
    clearInterval(pollTimer);
    pollTimer = setInterval(() => {
      if (document.visibilityState === 'visible' && isMarek()) pull();
    }, POLL_MS);
  }

  function watchUI() {
    observer?.disconnect();
    observer = new MutationObserver(() => requestAnimationFrame(applyReadOnly));
