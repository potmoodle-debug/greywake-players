(() => {
  ...
  const headers = () => ({ apikey: API_KEY, 'Content-Type':'application/json', 'x-greywake-character':'gm', 'x-greywake-code':'GREYWAKE' });
  ...
  async function revokeReveal(id) {
    const response = await fetch(API_URL, { method:'DELETE', headers:headers(), body:JSON.stringify({ id }) });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data.error || 'Could not remove reveal.');
    return true;
  }
  ...
  function ensureStyles() {
    ...
    const style = document.createElement('style'); style.id = 'gm-recent-reveals-styles'; style.textContent = `
      ...
      .gm-recent-reveals-list{display:grid;gap:7px}.gm-recent-reveal{display:grid;grid-template-columns:110px minmax(0,1fr) auto auto;gap:10px;align-items:center;padding:9px 10px;border:1px solid #353329;background:#11120e}.gm-recent-reveal-meta{color:#9c8c61;font-size:8px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.gm-recent-reveal-text{color:#d8cfb8;font-size:10px;line-height:1.35;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.gm-recent-reveal-time{color:#7f7868;font-size:9px;font-variant-numeric:tabular-nums}.gm-recent-reveal-remove{border:1px solid #5a4935;background:#17130f;color:#bd9d78;padding:5px 7px;font-size:7px;font-weight:900;letter-spacing:.06em;text-transform:uppercase;cursor:pointer}.gm-recent-reveal-remove:hover{border-color:#8a5a44;color:#ddb096}.gm-recent-reveal-remove:disabled{opacity:.45;cursor:wait}.gm-recent-reveals-empty{padding:10px 0;color:#88806e;font-size:10px}
      @media(max-width:700px){.gm-recent-reveal{grid-template-columns:1fr auto auto}.gm-recent-reveal-meta{grid-column:1}.gm-recent-reveal-text{grid-column:1/-1;white-space:normal}.gm-recent-reveal-time{grid-column:2;grid-row:1}.gm-recent-reveal-remove{grid-column:3;grid-row:1}}
  ...
  function render(items) {
    ...
    panel.innerHTML = `<div class="gm-recent-reveals-head"><div><small>RECENT REVEALS</small><strong>What the players have just been told</strong></div><span>Latest ${recent.length}</span></div>${recent.length ? `<div class="gm-recent-reveals-list">${recent.map(item => `<div class="gm-recent-reveal" data-reveal-id="${esc(item.id)}"><div class="gm-recent-reveal-meta">${esc(formatAudience(item.audience))} · ${esc(KINDS[item.reveal_kind] || item.reveal_kind)}</div><div class="gm-recent-reveal-text" title="${esc(item.body)}">${esc(item.body)}</div><div class="gm-recent-reveal-time">${esc(formatTime(item.created_at))}</div><button type="button" class="gm-recent-reveal-remove" data-remove-reveal="${esc(item.id)}" title="Remove this reveal from player-facing live reveals">Remove</button></div>`).join('')}</div>` : '<div class="gm-recent-reveals-empty">No live reveals sent yet.</div>'}`;
    panel.querySelectorAll('[data-remove-reveal]').forEach(button => button.addEventListener('click', async () => {
      const id = Number(button.dataset.removeReveal);
      if (!Number.isInteger(id)) return;
      const row = button.closest('.gm-recent-reveal');
      const text = row?.querySelector('.gm-recent-reveal-text')?.textContent?.trim() || 'this reveal';
      if (!window.confirm(`Remove this player reveal?\n\n${text}`)) return;
      button.disabled = true; button.textContent = 'Removing…';
      try { await revokeReveal(id); await refresh(); }
      catch (error) { button.disabled = false; button.textContent = 'Remove'; window.alert(error instanceof Error ? error.message : 'Could not remove reveal.'); }
    }));
  }
