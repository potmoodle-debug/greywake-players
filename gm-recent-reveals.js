(() => {
  ...
  function inSessionSupport() {
    return location.hash === '#/gm-session';
  }
  function esc(value) {
    return String(value ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]));
  }
  function formatAudience(list) {
    const values = Array.isArray(list) ? list : [];
    if (values.includes('party')) return 'Party';
    return values.map(x => AUDIENCE[x] || x).join(' · ') || 'Unknown';
  }
  ...
  function ensureStyles() {
    ...
    style.id = 'gm-recent-reveals-styles';
    style.textContent = `
  ...
  function host() {
    return document.getElementById('gmOperationsView')?.querySelector('.gm-ops-grid') || null;
  }
  function render(items) {
    if (!fullGM() || !inSessionSupport()) {
      document.getElementById('gmRecentReveals')?.remove();
      return;
    }
    const grid = host();
    if (!grid) return;
    ...
    if (!panel) {
      ...
      panel.id = 'gmRecentReveals';
      panel.className = 'gm-recent-reveals';
      grid.prepend(panel);
    }
    const recent = items.slice(0,5);
    panel.innerHTML = `
      <div class="gm-recent-reveals-head"><div><small>RECENT REVEALS</small><strong>What the players have just been told</strong></div><span>Latest ${recent.length}</span></div>
      ${recent.length ? `<div class="gm-recent-reveals-list">${recent.map(item => `<div class="gm-recent-reveal"><div class="gm-recent-reveal-meta">${esc(formatAudience(item.audience))} · ${esc(KINDS[item.reveal_kind] || item.reveal_kind)}</div><div class="gm-recent-reveal-text" title="${esc(item.body)}">${esc(item.body)}</div><div class="gm-recent-reveal-time">${esc(formatTime(item.created_at))}</div></div>`).join('')}</div>` : '<div class="gm-recent-reveals-empty">No live reveals sent yet.</div>'}`;
  }
  async function refresh() {
    if (!fullGM() || !inSessionSupport()) { render([]); return; }
    try { render(await getReveals()); } catch (_) { /* keep play uninterrupted */ }
  }
  function resetTimer() {
    if (timer) clearInterval(timer);
    timer = null;
    refresh();
    if (fullGM() && inSessionSupport()) timer = setInterval(refresh, POLL_MS);
  }
  window.addEventListener('hashchange', () => setTimeout(resetTimer, 0));
  window.addEventListener('greywake:player-ready', () => setTimeout(resetTimer, 0));
  window.addEventListener('greywake:live-reveal-published', () => setTimeout(refresh, 0));
