(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .player-mind-view{padding:38px clamp(22px,5vw,75px) 34px;border-bottom:1px solid var(--line);background:linear-gradient(180deg,#15150f 0%,#11120e 100%)}
      .player-mind-view .section-head{align-items:center;margin-bottom:16px}.player-mind-view .section-head p{max-width:560px;font-size:12px}
      .player-mind-summary{margin:-2px 0 17px;color:#8f8875;font-size:10px;line-height:1.55;border-left:2px solid #756a43;padding-left:12px}
      .player-mind-count{display:inline-flex;align-items:center;margin-top:8px;padding:5px 8px;border:1px solid rgba(210,190,128,.42);background:rgba(16,17,13,.76);color:#e4d298;font-size:8px;font-weight:800;letter-spacing:.13em;text-transform:uppercase}
      .player-mind-grid{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px}
      .player-mind-card{position:relative;min-height:172px;display:flex;flex-direction:column;justify-content:flex-end;isolation:isolate;border:1px solid #3a3b2e;background:#171811;overflow:hidden;box-shadow:0 12px 30px rgba(0,0,0,.18);transition:transform .22s ease,border-color .22s ease,box-shadow .22s ease;cursor:pointer;text-align:left;color:inherit;padding:0}
      ...
      .player-mind-card-content{position:relative;z-index:1;padding:14px 14px 13px}
      ...
      .player-mind-card h3{font:18px/1.1 Georgia,serif;margin:7px 0 7px;color:#f0e7ce;text-wrap:balance;text-shadow:0 2px 12px rgba(0,0,0,.78)}
      ...
      .player-mind-open{display:inline-block;margin-top:8px;color:#dcc989;font-size:7px;font-weight:800;letter-spacing:.11em;text-transform:uppercase}
      .player-mind-empty{min-height:172px;border:1px dashed #3e3c31;background:linear-gradient(180deg,#171811,#13140f);display:flex;align-items:flex-end;padding:14px;text-align:left;color:#777261}
      ...
      @media(max-width:520px){.player-mind-grid{grid-template-columns:1fr}.player-mind-card,.player-mind-empty{min-height:150px}.player-mind-view{padding-top:30px}.player-mind-view .section-head{display:block}.player-mind-summary{margin-top:10px}}
  ...
  function render() {
    ...
    minds.forEach((card, index) => {
      ...
      cards.push(`<button type="button" class="player-mind-card" data-player-mind-goal="${escapeHTML(id)}">${visual}<span class="player-mind-card-content"><span class="player-mind-topline"><span class="player-mind-state">${escapeHTML(state)}</span><span class="player-mind-slot">${index + 1}/5</span></span><h3>${escapeHTML(title)}</h3>${source ? `<span class="player-mind-source">${escapeHTML(source)}</span>` : ''}<span class="player-mind-open">Open →</span></span></button>`);
    });
    ...
    section.innerHTML = `<div class="section-head"><div><div class="eyebrow">YOUR CURRENT PRIORITIES</div><h2>What's on my mind</h2><div class="player-mind-count">${minds.length}/5 ACTIVE</div></div><p>A quick reference for the things currently at the front of your character's mind.</p></div><p class="player-mind-summary">These are interests, not promises. Add or change them as your character's attention changes.</p><div class="player-mind-grid">${cards.join('')}</div>`;
