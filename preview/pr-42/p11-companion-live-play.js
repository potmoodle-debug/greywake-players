(() => {
  ...
  const resources=()=>window.GreywakeCompanion?.getState?.()||null;
  ...
  function ensureWaterRow(){
    ...
    let row=board.querySelector('.live-resource-water');const current=water();
    ...
    row.querySelectorAll('[data-p11-water-delta]').forEach(b=>b.addEventListener('click',()=>setWater(current+Number(b.dataset.p11WaterDelta||0))));
    row.querySelectorAll('[data-p11-water-value]').forEach(b=>b.addEventListener('click',()=>{const v=Number(b.dataset.p11WaterValue||0);setWater(current===v?v-1:v);}));
  }
  ...
  function ensureArmorRow(){
    ...
    row.querySelectorAll('[data-p11-armor-value]').forEach(b=>b.addEventListener('click',()=>{const v=Number(b.dataset.p11ArmorValue||0);setArmor(current===v?v-1:v);}));
  }

  function ensureRestUtility(){
    const title=document.querySelector('#characterSheet .live-resource-board .pro-board-title');if(!title)return;
    let host=title.querySelector('.p11-rest-utility');
    if(!host){
      host=document.createElement('div');host.className='p11-rest-utility';host.setAttribute('aria-label','Rest controls');
      ...
      host.querySelector('[data-p11-short-rest]')?.addEventListener('click',()=>window.GreywakeRest?.openShort?.());
      host.querySelector('[data-p11-long-rest]')?.addEventListener('click',()=>window.GreywakeRest?.openLong?.());
  ...
  function ensureFieldActions(){
    const board=document.querySelector('#characterSheet .live-resource-board');if(!board)return;
    let host=board.querySelector('.p11-field-actions');if(!host){host=document.createElement('div');host.className='p11-field-actions';board.appendChild(host);}
    ...
    host.querySelector('[data-p11-take-damage]')?.addEventListener('click',()=>window.GreywakeDamage?.openDamage?.());
    host.querySelector('[data-p11-can-do]')?.addEventListener('click',openCanDo);
    host.querySelector('[data-p11-backpack]')?.addEventListener('click',()=>window.GreywakeBackpack?.open?.());
  }
  ...
  function ensureEvasionReadout(){
    ...
    if(mark.classList.contains('p11-evasion-mark')&&mark.dataset.evasion===value)return;
  ...
  function removeDuplicates(){
    ...
    const traits=[...document.querySelectorAll('#characterSheet .sheet-group')].find(g=>g.querySelector('.sheet-group-head h3')?.textContent.trim()==='Traits');
    traits?.classList.add('p11-hidden-duplicate');
  }

  function positionQuickRolls(){
    const traits=document.getElementById('traitRollPanel'),resourcesBoard=document.querySelector('#characterSheet .live-resource-board');
    if(traits&&resourcesBoard&&traits.nextElementSibling!==resourcesBoard)resourcesBoard.insertAdjacentElement('beforebegin',traits);
  }
  ...
  function openActionUse(title){
    ...
    const wait=()=>{const detail=panel.querySelector('.active-action-detail');if((!detail||!detail.querySelector('button,input,select,.action-roller'))&&tries++<12){setTimeout(wait,25);return;}if(!detail)return;
      const d=ensureActionDialog();d.innerHTML=`<div class="p11-dialog-shell"><div class="equip-dialog-head"><div><span>USE NOW</span><h2>${esc(title)}</h2></div><button type="button" data-close>×</button></div><div class="p11-action-use-body"></div></div>`;d.querySelector('.p11-action-use-body')?.appendChild(detail);d.querySelector('[data-close]')?.addEventListener('click',closeActionUse);d.addEventListener('cancel',e=>{e.preventDefault();closeActionUse();},{once:true});if(typeof d.showModal==='function'&&!d.open)d.showModal();else d.setAttribute('open','');
    };wait();
  }

  function openCanDo(){
    const panel=document.getElementById('activeActionsPanel'),d=ensureCanDoDialog();if(!panel)return;
    const cards=[...panel.querySelectorAll('.active-action-card')].filter(available);
    ...
    d.querySelectorAll('[data-p11-action-title]').forEach(b=>b.addEventListener('click',()=>{const title=b.dataset.p11ActionTitle;d.close();openActionUse(title);}));
  ...
  function repair(){if(repairing||!active()||!onCharacterRoute())return;repairing=true;try{refresh();}finally{setTimeout(()=>{repairing=false;},25);}}
  function verify(){if(!active()||!onCharacterRoute())return;if(needsRepair())repair();else{ensureEvasionReadout();removeDuplicates();positionQuickRolls();}}
  function watch(){if(!active()||!onCharacterRoute())return;const root=document.getElementById('characterSheet');if(!root){timer=setTimeout(watch,120);return;}if(root!==observedRoot){observer?.disconnect();observedRoot=root;observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(verify,30);});observer.observe(root,{childList:true,subtree:true});}verify();}
  function schedule(){clearTimeout(timer);timer=setTimeout(watch,60);}
  ...
  setTimeout(schedule,150);setTimeout(schedule,500);setTimeout(schedule,1200);
