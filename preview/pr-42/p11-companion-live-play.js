(() => {
  ...
  const actionsPanel=()=>document.getElementById('companionActionsPanel')||document.getElementById('activeActionsPanel');
  ...
  function ensureWaterRow(){
    ...
    const signature=String(current);if(row.dataset.p11Signature===signature)return;row.dataset.p11Signature=signature;
  ...
  function ensureArmorRow(){
    ...
    const score=Math.max(0,Number(c?.armorScore||stat('Armor'))||0),current=Math.max(0,Math.min(score,Number(d.armorMarked)||0));let row=board.querySelector('.live-resource-armor');
    ...
    const signature=`${current}/${score}`;if(row.dataset.p11Signature===signature)return;row.dataset.p11Signature=signature;
  ...
  function ensureRestUtility(){
    ...
    host.innerHTML='<button type="button" data-p11-short-rest><span aria-hidden="true">◐</span>Short Rest</button><button type="button" data-p11-long-rest><span aria-hidden="true">☾</span>Long Rest</button>';title.appendChild(host);
  }
  ...
  function openCanDo(){
    const panel=actionsPanel(),d=ensureCanDoDialog();if(!panel)return;const cards=[...panel.querySelectorAll('.active-action-card')].filter(available);
  ...
  function preserveResourceScroll(e){
    if(!active())return;const t=e.target.closest?.('[data-companion-adjust],[data-companion-set],[data-companion-undo]');if(!t)return;const y=window.scrollY;
  ...
  function watch(){if(!active()||!onCharacterRoute())return;const root=document.getElementById('characterSheet');if(!root){timer=setTimeout(watch,120);return;}if(root!==observedRoot){observer?.disconnect();observedRoot=root;root.addEventListener('click',preserveResourceScroll,true);root.addEventListener('click',handleClick);observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(verify,70);});observer.observe(root,{childList:true,subtree:true});}verify();}
  function schedule(){clearTimeout(timer);timer=setTimeout(watch,70);}
  ...
  window.addEventListener('hashchange',schedule);document.addEventListener('DOMContentLoaded',schedule);setTimeout(schedule,160);setTimeout(schedule,550);setTimeout(schedule,1200);
