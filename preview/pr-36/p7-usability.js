(() => {
  ...
  function ensureUtilityStyles() {
    ...
    style.textContent = `
      ...
      .p7-utility-card{border:1px solid rgba(202,179,111,.22);background:rgba(14,14,11,.5);padding:14px}.p7-utility-card>span{font-size:9px;letter-spacing:.14em;color:#9d947b;font-weight:800}.p7-utility-card>strong{display:block;margin:4px 0 10px;color:#ead79e}
      ...
      .p7-list{display:flex;flex-wrap:wrap;gap:7px;margin-top:10px}.p7-chip{display:inline-flex;align-items:center;border:1px solid rgba(202,179,111,.25);background:#181711}.p7-chip span{padding:7px 9px;font-size:12px}.p7-chip button{border:0;border-left:1px solid rgba(202,179,111,.2);padding:7px 9px}.p7-empty{font-size:11px;color:#8e8774}
      .p7-action-effect{border:1px solid #756642;background:#181711;padding:14px;margin:12px 0}.p7-action-effect strong{display:block;color:#f1d68b;margin-bottom:5px}.p7-action-effect p{margin:4px 0;color:#cfc5a5}.p7-action-effect select{background:#12120f;color:#eee3bf;border:1px solid #5e553c;padding:8px;margin:8px 6px 8px 0}.p7-action-effect button{border:1px solid #9b7b42;background:#2d2517;color:#f1d68b;padding:9px 12px;cursor:pointer}.p7-action-result{margin-top:8px;font-size:12px;color:#ddd1ad}#qnaQuickBtn{white-space:nowrap}
  ...
  function renderUtilities(force = false) {
    ...
    root.querySelectorAll('[data-p7-kind]').forEach(bindUtilityCard);
  ...
  function ensureQnaAccess() {
    ...
    button.id = 'qnaQuickBtn'; button.type = 'button'; button.textContent = 'Q&A';
    button.title = 'Questions & replies'; button.setAttribute('aria-label','Open Questions and replies');
  ...
  function openFormPicker(mode) {
    (document.getElementById('chooseBeastform') || document.getElementById('changeBeastform'))?.click();
    setTimeout(() => {
  ...
  function replaceActionDetail(title) {
    ...
    detail.querySelector('.p7-action-effect')?.remove();
    const tools = detail.querySelector('.active-action-detail-tools');
    const panel = document.createElement('section');
    panel.className = 'p7-action-effect'; detail.dataset.p7Effect = title;
    if (title === 'Clarity of Nature') {
      ...
      panel.querySelector('[data-regen-roll]')?.addEventListener('click', () => {
        ...
        if (target === 'Marek' && resources?.getState && resources?.setResource) {
          const state = resources.getState(); const marked = Number(state.hp || 0); const cleared = Math.min(roll, marked);
          resources.setResource('hp', Math.max(0, marked-cleared), `Regeneration · ${roll}`); applied = ` Marek clears ${cleared} marked HP.`;
        } else applied = ` ${target} should clear up to ${roll} marked HP on their sheet.`;
  ...
  function handleActionClick(event) {
    ...
    if (title === 'Beastform' || title === 'Evolution') {
      event.preventDefault(); event.stopImmediatePropagation(); openFormPicker(title === 'Evolution' ? 'evolution' : 'stress'); return;
    }
    if (title === 'Regeneration' || title === 'Clarity of Nature') setTimeout(() => replaceActionDetail(title), 0);
  }
  ...
  async function currentGoalFor(wrap) {
    ...
    const key = String(wrap.dataset.contextMind || ''), title = contextTitle(wrap).toLowerCase();
  ...
  function makePriorityControlsReversible() {
    ...
    document.querySelectorAll('.context-mind-action').forEach(wrap => {
      ...
      if (interest && !interest.dataset.p7Reversible) { interest.disabled = false; interest.dataset.p7Reversible = 'interest'; interest.title = 'Remove from Interested'; }
      if (pursue && !pursue.dataset.p7Reversible) { pursue.disabled = false; pursue.dataset.p7Reversible = 'pursuing'; pursue.title = 'Stop pursuing'; }
    });
  }
  ...
  async function handlePriorityClick(event) {
    ...
    const wrap = button.closest('.context-mind-action'); if (!wrap) return;
    ...
    try {
      const goal = await currentGoalFor(wrap); if (!goal) throw new Error('Could not find that interest.');
  ...
  function observePriorities() {
    priorityObserver?.disconnect();
    const root = document.getElementById('currentThreads') || document.getElementById('home');
    if (!root) return;
    priorityObserver = new MutationObserver(() => requestAnimationFrame(makePriorityControlsReversible));
    priorityObserver.observe(root,{childList:true,subtree:true});
  }

  function init() {
    clearTimeout(initTimer);
    ensureUtilityStyles(); ensureQnaAccess(); wrapEquipmentSync(); renderUtilities(); makePriorityControlsReversible(); observePriorities();
    initTimer = setTimeout(() => { wrapEquipmentSync(); renderUtilities(); makePriorityControlsReversible(); }, 250);
  }
