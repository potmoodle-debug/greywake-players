(() => {
  ...
  function renderPrep(){
    ...
    const cards=players.map(([key,name,image])=>{
      ...
      const state=item?.status==='pursuing'?'PURSUING':item?.kind==='question'?'QUESTION':item?'PLAYER INTEREST':'NONE CURRENT';
